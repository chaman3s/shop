...
import { startLoginWithOtp } from "@/lib/services";
...
export async function POST(request: Request) {
  ...
  const result = await startLoginWithOtp(body.email, body.password);
  if ("error" in result) {
  ...
  if (!emailResult.sent) {
    console.log(`[mock-email] Login OTP for ${result.email}: ${result.otp}`);
  }
